...
{
	assert(time >0.5f);
	//Mark a source to make a fadeout
	//Get the gain and calculate a rate
	float gain (1.0f);
	GetSourceGain(source,gain);
	float rate (gain / (1000 * time));  //Rate
	mFadeOutSources.push_back(FadeOutInfo(source,rate,false));
}
...
void SoundManager::FadeOutMusic(const std::string& name, float time = 2.0)
{
	...
	GetSourceGain(source,gain);
	float rate (gain / (1000 * time));  //Rate
	mFadeOutSources.push_back(FadeOutInfo(source,rate,true));
}
