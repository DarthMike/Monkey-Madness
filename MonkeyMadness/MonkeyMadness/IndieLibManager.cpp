...
void IndieLibManager::_init()
{
	...
	if(!mSystemInitialized)
	{
		...
		if (!Window				->Init ("MMGame", graphicsconf.resx, graphicsconf.resy, 32, graphicsconf.vsync, graphicsconf.fullscreen))
		...
		Camera2DPointer overlaycam(new OverlayCamera2D(Vector2(0,0),mGlobalScale,63,Window->GetWidth(),Window->GetHeight(),"Overlay",mMaxAABB,mMinAABB));  //Layer 63 (in top)
