...
//*****************************DESTRUCTION LISTENER IMPLEMENTATION********************************
//Implicit destruction of a Joint
void GameDestructionListener::SayGoodbye(b2Joint* joint)
{
	//Search joint in joint map and destroy it
	PhysicsManager::JointsMapIterator itr;

	//LOOP - Search destructed joint
	for(itr = mPhysicsMgr->mJointsMap.begin(); itr != mPhysicsMgr->mJointsMap.begin(); ++itr)
	{
		//IF - Found
		if((*itr).second == joint)
		{
			mPhysicsMgr->mJointsMap.erase(itr);
			//Add event
			mPhysicsMgr->mDestroyedJoints.push_back(PhysicsManager::DestroyedJointData((*itr).second,(*itr).first));
			break;
		}//IF
	}//LOOP 
}

void GameDestructionListener::SayGoodbye(b2Shape*)
{
	//For now, nothing
}
...
void PhysicsManager::Update(float dt)
{
	...
	//---------------------Send "destroyed joints" events-------------------------
	DestroyedJointsVecIterator jointitr;
	//LOOP - Send all messages for joints destruction
	for(jointitr = mDestroyedJoints.begin(); jointitr != mDestroyedJoints.end(); jointitr++)
	{
		//Call method to send event
		_sendDestroyedJointEvent((*jointitr));
	}//LOOP END

	//Clear container
	mDestroyedJoints.clear();
}
...
b2Body* PhysicsManager::GetBody(const std::string& name)
{
	...
	//IF - Empty name -  return ground body
	if(name == "")
	{
		return mpTheWorld->GetGroundBody();
	}//IF
	
	//find it if not empty (ground body)
...
{
	//Be sure joint doesnt exist already
...
b2Joint* PhysicsManager::CreateDistanceJoint(b2DistanceJointDef* definition, const std::string& jointname, const std::string& body1, const std::string& body2, const b2Vec2& worldpoint1, const b2Vec2& worldpoint2)
{
...
b2Joint* PhysicsManager::CreateRevoluteJoint(b2RevoluteJointDef* definition, const std::string& jointname, const std::string& body1, const std::string& body2, const b2Vec2& worldpoint)
{
	//Name of joint has to be coherent
	JointsMapIterator jointitr = mJointsMap.find(jointname);
	//IF - Name of joint is not correct
...
b2Joint* PhysicsManager::CreatePrismaticJoint(b2PrismaticJointDef* definition, const std::string& jointname, const std::string& body1, const std::string& body2, const b2Vec2& worldpoint, const b2Vec2& axis)
{
	//Name of joint has to be coherent
	JointsMapIterator jointitr = mJointsMap.find(jointname);
	//IF - Name of joint is not correct
...
b2Joint* PhysicsManager::CreatePulleyJoint(b2PulleyJointDef* definition, const std::string& jointname, const std::string& body1, const std::string& body2)
{
	//Name of joint has to be coherent
	JointsMapIterator jointitr = mJointsMap.find(jointname);
	//IF - Name of joint is not correct
...
b2Joint* PhysicsManager::CreateFixedJoint(b2FixedJointDef* definition, const std::string& jointname)
{
	//Name of joint has to be coherent
	JointsMapIterator jointitr = mJointsMap.find(jointname);
	//IF - Name of joint is not correct
	...
	{
		//IF - Bodies are correct
		if(GetBodyName(definition->body1) != "" && GetBodyName(definition->body2) != "")
		{
			b2Joint* newjoint = mpTheWorld->CreateJoint(definition);
			if(newjoint)
			{
...
//Events generation - Destroyed Joint
void PhysicsManager::_sendDestroyedJointEvent(const DestroyedJointData& destroyedjoint)
{
	//Construct event data
	DestroyedJointInfo data(destroyedjoint.first,destroyedjoint.second);
	//Send event
	SingletonGameEventMgr::Instance()->TriggerEvent(
						EventDataPointer( new DestroyedJointEvent(Event_DestroyedJoint,data))
						);
}
...
void PhysicsManager::_release()
{
	...
	//Delete destruction listener
	if(mpDestructionListener)
	{
		delete mpDestructionListener;
		mpDestructionListener = NULL;
	}
