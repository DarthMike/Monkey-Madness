...
//-----------------------------Custom Destruction listener----------------------------------- 
class PhysicsManager;
class GameDestructionListener : public b2DestructionListener 
{
public:
	//----- CONSTRUCTORS/DESTRUCTORS -----
	GameDestructionListener(PhysicsManager* managerptr)
		:mPhysicsMgr(managerptr)
	{}
	~GameDestructionListener()
	{}
public:
	//----- OTHER FUNCTIONS -----
	//IMPLEMENTATION OF VIRTUAL INTERFACE
...
class PhysicsManager
{
	...
	friend class GameDestructionListener;
	//Definitions
public:
...
protected:
	...
	//container for destroyed joints
	typedef std::pair<b2Joint*,std::string> DestroyedJointData;
	typedef std::vector<DestroyedJointData> DestroyedJointsVec;
	typedef DestroyedJointsVec::iterator DestroyedJointsVecIterator;
...
public:
		 ...
		 mTimeStepped(0.0f)
	{
		...
		//Config destruction listener
		mpDestructionListener = new GameDestructionListener(this);
		mpTheWorld->SetDestructionListener(mpDestructionListener);
	...
	b2Joint* CreateDistanceJoint(b2DistanceJointDef* definition, const std::string& jointname, const std::string& body1, const std::string& body2, const b2Vec2& worldpoint1, const b2Vec2& worldpoint2);
	b2Joint* CreateRevoluteJoint(b2RevoluteJointDef* definition, const std::string& jointname, const std::string& body1, const std::string& body2, const b2Vec2& worldpoint);
	b2Joint* CreatePrismaticJoint(b2PrismaticJointDef* definition, const std::string& jointname, const std::string& body1, const std::string& body2, const b2Vec2& worldpoint, const b2Vec2& axis);
	b2Joint* CreatePulleyJoint(b2PulleyJointDef* definition, const std::string& jointname, const std::string& body1, const std::string& body2);
	b2Joint* CreateGearJoint(b2GearJointDef* definition, const std::string& jointname, const std::string& joint1, const std::string& joint2);
	b2Joint* CreateFixedJoint(b2FixedJointDef* definition, const std::string& jointname);
...
protected:
	...
	GameDestructionListener* mpDestructionListener;  //Destruction listener implementation
	...
	DestroyedJointsVec mDestroyedJoints;  //Container to know which joints were destroyed
	//----- INTERNAL FUNCTIONS -----
	void _release();
	...
	void _sendDestroyedJointEvent(const DestroyedJointData& destroyedjoint);
